...
public class LogAnalyzerWindowed implements Serializable {
  ...
  public void processAccessLogs(JavaDStream<ApacheAccessLog> accessLogsDStream) {
	  System.out.println("In log analyzer window");
    ...
    windowDStream.foreachRDD(accessLogs -> {
      if (accessLogs.count() > 0) {
        ...
        List<Tuple2<Integer, Long>> ipAddress =
                Functions.responseCodeCount(accessLogs).collect();
                   // .take(100);
        
        JavaPairRDD<String, Long> ipAddressCounts =
            Functions.ipAddressCount(accessLogs);
        List<String> ipAddresses = Functions.filterIPAddress(ipAddressCounts).collect();
            //.take(100);
