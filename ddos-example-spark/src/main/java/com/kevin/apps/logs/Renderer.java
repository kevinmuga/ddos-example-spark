...
import com.kevin.kafka.pojo.IPInfo;
...
public class Renderer implements Serializable {
  ...
  public void render(LogStatistics allOfTime, LogStatistics lastWindow) throws IOException {
    ...
    writeOutput(output);
    writeToDirectory(allOfTime, lastWindow);
  }
  ...
  private void writeOutput(String output) throws IOException {
	...
	public void writeToDirectory(LogStatistics allOfTime, LogStatistics lastWindow) throws IOException {

        ipAddressCount(allOfTime, lastWindow);
}



	private void ipAddressCount(
	      LogStatistics allOfTime, LogStatistics lastWindow) {
	    Map<String, IPInfo> ipInfoMap = new HashMap<String, IPInfo>();
	    List<String> totalIpAddresses = allOfTime.getIpAddresses();
//	    System.out.println("totalIpAddress Count: "+totalIpAddresses.size());

	    
	    try {
			generateDDoSFile(totalIpAddresses);
		} catch (IOException e) {
	...
	public static void generateDDoSFile(List<String> ipInfoList) throws IOException {
		System.out.println("In generateDDoSFile 1");
		if (ipInfoList != null && !ipInfoList.isEmpty()){
			System.out.println("In generateDDoSFile 2");
			System.out.println("ipInfoList size: "+ipInfoList.size());
			
			File fout = new File("/Users/kevinmuga/Documents/workspace/FileStorage/Output/DDOS-out1.txt");
